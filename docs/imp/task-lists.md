...
- [ ] 4エージェントを MoChat に `selfRegister` する
- [ ] 各エージェントを `bind` で自分のメールに紐付ける
- [ ] 以前露出した可能性があるトークンを `rotateToken` で再発行する

## エージェント実施タスク

- [ ] `config/local/` 前提の secret 管理手順を文書で固定する
- [ ] 討論停止条件（合意 / 対立 / 保留）を `protocol` と仕様に明文化する
- [ ] 停滞検知（反復 / 逸脱 / 品質低下）の閾値案を定義する
- [ ] 結果保存先を `docs/debate-results/` に固定し、出力テンプレートを作る
- [ ] internal 命名（`mediator`）と表示名（「メドラー」）の使い分けを統一する

## 共同作業タスク（ユーザー + エージェント）

- [ ] 討論セッションを `sessions/create` で作成し、実行テストを行う
- [ ] 設計書レビュー依頼を `sessions/send` で投稿し、1ラウンド完走を確認する
