...
- `config/local/` 配下や `.env.local` など、secret を含むファイルは通常作業で参照しない。
- secret ファイルへのアクセスは、認証設定・接続確認・トークン更新など、必要性が明確なタイミングに限定する。
- ドキュメント更新、仕様検討、通常実装では secret の実値を読まないことを原則とする。
- secret の実値を会話・ログ・成果物へ転記しない。
